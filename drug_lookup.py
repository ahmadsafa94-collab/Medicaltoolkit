...
def _sanitize_for_telegram_markdown(text: str) -> str:
    """
    Strip characters that break Telegram's legacy Markdown parser when they
    appear unmatched -- FDA label text frequently has stray '*' (footnote
    markers), '_' (used in drug/gene names), or brackets that aren't meant
    as Markdown formatting but get misread as it, silently failing the send.
    """
